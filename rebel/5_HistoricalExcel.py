...
output_file = BASE_DIR / f"Rebel_Historical_{london_date_str}_({london_time_str}).xlsx"

# ---------- Step 1.5: Remove today's previous Excel files ----------
today_pattern = str(BASE_DIR / f"Rebel_Historical_{london_date_str}_(*).xlsx")

for file_path in glob.glob(today_pattern):
    try:
        os.remove(file_path)
        print(f"Deleted previous Rebel file for today: {file_path}")
...
df = df.rename(columns={
    "Concept Name": "Service/Class",
    "Location/Club": "Staff/Suite."
})
...
print(f"\n Excel file '{output_file.name}' created successfully with merged Date cells.")
